...
@allure.title('Test Patch User')
@allure.description('This test verifies that an existing user can be patched successfully.')
@allure.severity(allure.severity_level.CRITICAL)
@pytest.mark.regression
@pytest.mark.parametrize("user_id, name, job", [
    (2, "morpheus", "zion resident")
])
def test_patch_user(user_id, name, job):
    ...
    with allure.step("Send PATCH request to update the user"):
